...
# --- 页面基础配置 ---
st.set_page_config(
    page_title="LOFTER AI模型去水印",
    page_icon="🎨",
    layout="wide"
)

st.title("🎨 LOFTER AI模型去水印")
...
with tab1:
    st.header("批量图片重命名")
    st.info("此工具用于将上传的LOFTER图片对，按时间顺序两两为一组，将每组大/小文件自动命名为 `[ID]-wm.jpg` 和 `[ID]-orig.jpg` 格式。不能保证时间顺序正确请一次上传两张，一张有水印一张无水印，会自动按大小命名。")
...
with tab2:
    ...
    with st.sidebar:
        ...
        st.subheader("搜索区域 (默认即可)(使用比例，0.0为最左/上，1.0为最右/下)")
